...
export function applyAppearance(prefs: AppearancePrefs) {
  ...
  const notchSize = clamp(chrome.notchSize ?? 64, 64, 160)
  const notchDepth = clamp(chrome.notchDepth ?? 59, 36, 96)
  const notchScoop = clamp(chrome.notchScoop ?? 19, 10, 40)
  const notchPop = clamp(chrome.notchPop ?? 11, 0, 36)
  const notchCircle = clamp(chrome.notchCircle ?? 39, 32, 64)
  const notchShadow = clamp(chrome.notchShadow ?? 12, 0, 18)
