using FattoVirtual.Domain.Enums;
using FattoVirtual.Infrastructure.Persistence;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;

namespace FattoVirtual.Api.Controllers;

[ApiController]
[Authorize]
[Route("api/v1/reports")]
public class ReportsController : ControllerBase
{
    private readonly AppDbContext _db;
    public ReportsController(AppDbContext db) => _db = db;

    [HttpGet]
    [RequirePermission(Permissions.Dashboard)]
    public async Task<IActionResult> Get(
        [FromQuery] string? lens,
        [FromQuery] string? period,
        [FromQuery] Guid? clientId)
    {
        var orgId = User.GetOrganizationId();
        var uid = User.GetUserId();
        var canAdm = User.IsOwner() || User.HasPermission(Permissions.FinanceAll);
        var resolvedLens = (lens ?? (canAdm ? "adm" : "va")).ToLowerInvariant();
        if (resolvedLens is not ("adm" or "va" or "client"))
            return BadRequest(new { detail = "Olhar inválido. Use adm, va ou client." });
        var resolvedPeriod = string.IsNullOrWhiteSpace(period) ? "week" : period;
        if (resolvedPeriod is not ("day" or "week" or "month"))
            return BadRequest(new { detail = "Período inválido. Use day, week ou month." });
        if (resolvedLens == "adm" && !canAdm) resolvedLens = "va";
        if (resolvedLens == "client" && clientId is null)
            return BadRequest(new { detail = "Relatório do cliente exige clientId." });

        var (from, to) = Range(resolvedPeriod);
        var clientsQ = ClientScope.Query(_db, User);
        if (clientId is Guid selectedId && !await clientsQ.AnyAsync(c => c.Id == selectedId))
            return NotFound(new { detail = "Cliente não encontrado." });
        var clientIds = clientsQ.Select(c => c.Id);
        if (clientId is Guid cid)
            clientIds = clientIds.Where(id => id == cid);

        var todosQ = _db.TodoItems.Where(t => t.OrganizationId == orgId && t.CreatedAt < to &&
            ((clientId == null && t.ClientId == null) || (t.ClientId != null && clientIds.Contains(t.ClientId.Value))));
        if (resolvedLens == "va")
            todosQ = todosQ.Where(t => t.AssignedUserId == uid || t.CreatedByUserId == uid);

        var doneTodosQ = todosQ.Where(t => t.Status == TodoStatus.Done && t.UpdatedAt >= from && t.UpdatedAt < to);
        var done = await doneTodosQ.CountAsync();
        var open = await todosQ.CountAsync(t => t.Status != TodoStatus.Done);
        var overdue = await todosQ.CountAsync(t =>
            t.Status != TodoStatus.Done && t.DueAtUtc != null && t.DueAtUtc < DateTime.UtcNow);

        var agendaQ = _db.AgendaEvents.Where(e => e.OrganizationId == orgId &&
            e.StartsAt >= from && e.StartsAt < to &&
            ((clientId == null && e.ClientId == null) || (e.ClientId != null && clientIds.Contains(e.ClientId.Value))));
        if (resolvedLens == "va")
            agendaQ = agendaQ.Where(e => e.OwnerUserId == uid);

        var meetings = await agendaQ.CountAsync(e => e.Kind == "Meeting");
        var events = await agendaQ.CountAsync();

        var decQ = _db.BusinessDecisions.Where(d => d.OrganizationId == orgId &&
            d.CreatedAt >= from && d.CreatedAt < to &&
            ((clientId == null && d.ClientId == null) || (d.ClientId != null && clientIds.Contains(d.ClientId.Value))));
        if (resolvedLens == "client")
            decQ = decQ.Where(d => d.VisibleToClient);
        if (resolvedLens == "va")
            decQ = decQ.Where(d => d.OwnerUserId == uid || d.CreatedByUserId == uid);
        var decisionsTotal = await decQ.CountAsync();
        var decisionsOpen = await decQ.CountAsync(d => d.IsOpen);

        var runsQ = _db.SopRuns.Where(r =>
            r.OrganizationId == orgId && r.CompletedAtUtc != null &&
            r.CompletedAtUtc >= from && r.CompletedAtUtc < to &&
            ((clientId == null && r.ClientId == null) || (r.ClientId != null && clientIds.Contains(r.ClientId.Value))));
        if (resolvedLens == "va") runsQ = runsQ.Where(r => r.StartedByUserId == uid);
        var sopRuns = await runsQ.CountAsync();

        var scopedTimeQ = _db.TimeEntries.Where(t => t.OrganizationId == orgId &&
            ((clientId == null && t.ClientId == null) || (t.ClientId != null && clientIds.Contains(t.ClientId.Value))));
        if (resolvedLens == "va")
            scopedTimeQ = scopedTimeQ.Where(t => t.UserId == uid);
        var timeQ = scopedTimeQ.Where(t => t.StartedAtUtc >= from && t.StartedAtUtc < to);
        var minutes = await timeQ.SumAsync(t => (int?)t.Minutes) ?? 0;

        var scopedClientsQ = clientsQ.Where(c => clientId == null || c.Id == clientId);
        var retainerHours = await scopedClientsQ.SumAsync(c => (decimal?)c.RetainerHoursPerMonth) ?? 0;
        var (monthFrom, monthTo) = Range("month");
        var retainerMinutes = await scopedTimeQ
            .Where(t => t.ClientId != null && t.Billable && t.StartedAtUtc >= monthFrom && t.StartedAtUtc < monthTo)
            .SumAsync(t => (int?)t.Minutes) ?? 0;

        var payQ = _db.Payments.Where(p => p.OrganizationId == orgId &&
            (p.PaidAt ?? p.DueAt ?? p.CreatedAt) >= from &&
            (p.PaidAt ?? p.DueAt ?? p.CreatedAt) < to &&
            ((clientId == null && p.ClientId == null) || (p.ClientId != null && clientIds.Contains(p.ClientId.Value))));

        static MoneySlice Slice(IEnumerable<Domain.Entities.Payment> list, string ledger)
        {
            var s = list.Where(p => (string.IsNullOrWhiteSpace(p.Ledger) ? "Agency" : p.Ledger) == ledger).ToList();
            return new(s.Where(p => p.IsSettled).Sum(p => p.Amount), s.Where(p => !p.IsSettled).Sum(p => p.Amount));
        }

        var canFinance = canAdm || (resolvedLens == "va" && User.HasPermission(Permissions.FinanceOwn));
        if (resolvedLens == "va")
            payQ = payQ.Where(p => p.Ledger == "AssistantPayout" && p.TargetUserId == uid);
        if (resolvedLens == "client")
            payQ = payQ.Where(p => p.Ledger != "AssistantPayout");
        var payments = canFinance ? await payQ.ToListAsync() : [];
        var agency = Slice(payments, "Agency");
        var ar = Slice(payments, "ClientAr");
        var ap = Slice(payments, "ClientAp");
        var payoutAll = Slice(payments, "AssistantPayout");
        var myPayout = Slice(payments.Where(p => p.TargetUserId == uid), "AssistantPayout");

        object? money = !canFinance ? null : resolvedLens switch
        {
            "client" => new { agency, clientAr = ar, clientAp = ap },
            "va" => new { payout = myPayout },
            _ => new
            {
                agency,
                clientAr = ar,
                clientAp = ap,
                payout = payoutAll,
                marginPaid = agency.Paid - payoutAll.Paid
            }
        };

        var scopedClients = await scopedClientsQ
            .Select(c => new { c.Id, c.Name, c.RetainerHoursPerMonth })
            .ToListAsync();
        var todoByClient = await doneTodosQ
            .GroupBy(t => t.ClientId)
            .Select(g => new { ClientId = g.Key, Count = g.Count() })
            .ToListAsync();
        var timeByClient = await timeQ
            .GroupBy(t => t.ClientId)
            .Select(g => new { ClientId = g.Key, Minutes = g.Sum(x => x.Minutes) })
            .ToListAsync();
        var byClient = scopedClients
            .Select(c => new
            {
                clientId = c.Id,
                name = c.Name,
                retainerHours = c.RetainerHoursPerMonth,
                todosDone = todoByClient.FirstOrDefault(x => x.ClientId == c.Id)?.Count ?? 0,
                minutes = timeByClient.FirstOrDefault(x => x.ClientId == c.Id)?.Minutes ?? 0
            })
            .OrderByDescending(x => x.todosDone)
            .Take(40)
            .ToList();

        var recentTodos = await doneTodosQ
            .OrderByDescending(t => t.UpdatedAt).Take(12)
            .Select(t => new { t.Id, t.Title, t.ClientId, clientName = t.Client != null ? t.Client.Name : null, at = t.UpdatedAt })
            .ToListAsync();

        var recentDecisions = await decQ.OrderByDescending(d => d.CreatedAt).Take(12)
            .Select(d => new { d.Id, d.Title, d.VisibleToClient, d.IsOpen, d.ClientId, clientName = d.Client != null ? d.Client.Name : null, d.CreatedAt })
            .ToListAsync();

        return Ok(new
        {
            lens = resolvedLens,
            period = resolvedPeriod,
            from,
            to,
            todos = new { done, open, overdue },
            agenda = new { meetings, events },
            decisions = new { total = decisionsTotal, open = decisionsOpen },
            sops = new { completedRuns = sopRuns },
            time = new
            {
                minutes,
                hours = Math.Round(minutes / 60m, 2),
                retainerHours,
                retainerUsedHours = Math.Round(retainerMinutes / 60m, 2)
            },
            money,
            byClient,
            evidence = new { todos = recentTodos, decisions = recentDecisions }
        });
    }

    private static (DateTime from, DateTime to) Range(string? period)
    {
        var now = DateTime.UtcNow;
        var today = now.Date;
        return (period ?? "week") switch
        {
            "day" => (today, today.AddDays(1)),
            "month" => (new DateTime(today.Year, today.Month, 1, 0, 0, 0, DateTimeKind.Utc),
                new DateTime(today.Year, today.Month, 1, 0, 0, 0, DateTimeKind.Utc).AddMonths(1)),
            _ => (today.AddDays(-(int)today.DayOfWeek), today.AddDays(7 - (int)today.DayOfWeek))
        };
    }

    private sealed record MoneySlice(decimal Paid, decimal Pending);
}
